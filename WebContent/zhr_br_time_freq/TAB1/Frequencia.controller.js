sap.ui.controller("zhr_br_time_freq.TAB1.Frequencia", {
  ...
  // this.get_Frequency_Data(oPernr, oPeriodo)
  },
  /**
  ...
  //
  // }
  get_Frequency_Data: function(oPernr, oPeriodo) {
    ...
    oData.read("/PeriodsSet('" + oPeriodo.getSelectedKey() + "')", null, null, false, function(oDataSuccess) {
      ...
      oData.read("/FrequencySet", {
        filters: oFilter,
        success: function(oData) {
          var json = new sap.ui.model.json.JSONModel(oData);
          var oTable = sap.ui.getCore().byId('idFreq').byId('idFrequencia');
          var oTemplate = sap.ui.getCore().byId('idFreq').byId('idRowBase');
          oTable.destroyItems();
...
//            oline.mAggregations.cells[7].setVisible(true);
            ...
            if (oSel.TpStatus == '01' && (oLogin.getText() != oPernr.getText()) && (sap.ui.getCore().byId('idLocked').getText() == '')) {
              ...
              oStatus.attachPress(function(oEvent) {
                sap.ui.getCore().byId("idFreq").getController().Aprov_HE(oEvent);
              });
            }
          });
        }
...
//  },
  ...
  set_Frequency_Data: function(oEvent) {
    ...
    var erro = false
//    jQuery.each(tResult.results, function(index) {
//      if (tResult.results[index].Mod == 'X' && oTable.getItems()[index].getCells()[6].getValue() != "9001") {
//        if (oTable.getItems()[index].getCells()[10].getSelected() == false && oTable.getItems()[index].getCells()[8].getValue() == ''
//        && oTable.getItems()[index].getCells()[9].getValue() == '') {
//          erro = true
//        }
//      }
//    });
...
//      sap.m.MessageBox.show("O período parcial ou integral da justificativa deve ser informado.", {
...
//    };
    ...
    for(var i=0; i<this.oChangeLines.length; i++){
      ...
      oData.update("/FrequencySet(Pernr='" + tResult.results[this.oChangeLines[i]].Pernr + "'," + "Datum=datetime'" + tResult.results[this.oChangeLines[i]].Datum + "',"
 ...
 + tResult.results[this.oChangeLines[i]].Enduz + "')", oUpdate, null, function(oSuccess) {
    ...
    oData.create("/EmployedSet", oCreate, null, function(oSuccess) {
      ...
      sap.ui.controller("zhr_br_time_freq.TAB1.Frequencia").get_Frequency_Data(sap.ui.getCore().byId('idmain').byId('idPernr'), sap.ui.getCore().byId('idmain').byId('idPeriodo'));
  ...
  get_Frequency_Files: function(oEvent, p_line, p_row) {
    ...
    var oRowFile = new sap.m.ColumnListItem("idRowFile", {
      cells: [ new sap.m.Label({
        ...
        press: function(oEvent) {
          sap.ui.controller("zhr_br_time_freq.TAB1.Frequencia").Files_Show(p_line.Datum, oAwart, this.getParent().getCells()[3].getText());
        }
      }), new sap.m.Button({
        type: sap.m.ButtonType.Transparent,
        ...
        visible: lv_enabled,
        press: function(oEvent) {
          sap.ui.controller("zhr_br_time_freq.TAB1.Frequencia").Files_Delete(p_line.Datum, oAwart, this.getParent());
        }
      }), ],
    });
...
//    oToolBar.addContent(new sap.m.Label());
    ...
    oFiles.setHeaderToolbar(oToolBar);
      ...
      var oCell3 = new sap.m.Button({
        ...
        press: function(oEvent) {
//        sap.ui.controller("zhr_br_time_freq.TAB1.Frequencia").Files_Create(vCrm, vDate, vAwart);
          sap.ui.controller("zhr_br_time_freq.TAB1.Frequencia").Files_Create(vCrm, p_line.Datum, oAwart);
        }
      });
  ...
  Files_Create: function(vCrm, vDate, vAwart) {
    ...
    if (vCrm == true) {
      ...
      if (oCRM.getValue() == '' && vCrm == true) {
        ...
        // Define Header para atualia��o Odata
  ...
  Aprov_HE: function(oEvent) {
    ...
    var oTemplate = new sap.m.ColumnListItem('TemplateAprovHEAba1', {
      cells: [new sap.m.Label({
        ...
        tooltip: 'Aprovar HE',
        press: [sap.ui.controller("zhr_br_time_freq.TAB1.Frequencia").eDialogInsert]
      }), new sap.m.Button({
        enabled: {
          path: 'AutorizadasDescr',
          formatter: function(value) {
        ...
        tooltip: 'Editar',
        press: [sap.ui.controller("zhr_br_time_freq.TAB1.Frequencia").eDialogEdit]
      }), new sap.m.Button({
        enabled: {
          path: 'AutorizadasDescr',
          formatter: function(value) {
        ...
        icon: sap.ui.core.IconPool.getIconURI('delete'),
        tooltip: 'Excluir',
        press: [sap.ui.controller("zhr_br_time_freq.TAB1.Frequencia").eConfDelete]
      })
  ...
  eConfDelete: function(oEvent) {
    ...
    var oDialogConfDelete = new sap.m.Dialog('IDConfDelete', {
      ...
      buttons: [new sap.m.Button({
        ...
        type: sap.m.ButtonType.Accept,
        press: function() {
          sap.ui.getCore().byId("idFreq").getController().DeleteApproval(oParent);
        }
      }),
      // Bot�o Cancelar
      new sap.m.Button({
    ...
    if (sData == '' || sBegtime == '' || sEndtime == '') {
      ...
      var oDialogConfInsert = new sap.m.Dialog('IDConfInsert', {
        ...
        buttons: [new sap.m.Button({
          ...
          type: sap.m.ButtonType.Accept,
          press: function() {
            sap.ui.getCore().byId("idFreq").getController().AddApproval();
          }
        }),
        // Bot�o Cancelar
        new sap.m.Button({
    ...
    if (sData == '' || sBegtime == '' || sEndtime == '') {
      ...
      var oDialogConfEdit = new sap.m.Dialog('IDConfEdit', {
        ...
        buttons: [new sap.m.Button({
          ...
          type: sap.m.ButtonType.Accept,
          press: function() {
            sap.ui.getCore().byId("idFreq").getController().EditApproval(oParent);
          }
        }),
        // Bot�o Cancelar
        new sap.m.Button({
  ...
  eDialogInsert: function(oEvent) {
    ...
    oDialogInsert = new sap.m.Dialog('IDDialogInsert', {
      ...
      buttons: [new sap.m.Button({
        ...
        type: sap.m.ButtonType.Accept,
        press: function() {
          sap.ui.getCore().byId("idFreq").getController().eConfInsert();
        }
      }), new sap.m.Button({
        text: "Cancelar",
        type: sap.m.ButtonType.Default,
  ...
  eDialogEdit: function(oEvent) {
    ...
    oDialogEdit = new sap.m.Dialog('IDDialogEdit', {
      ...
      buttons: [new sap.m.Button({
        ...
        type: sap.m.ButtonType.Accept,
        press: function() {
          sap.ui.getCore().byId("idFreq").getController().eConfEdit(oParent);
        }
      }), new sap.m.Button({
        text: "Cancelar",
        type: sap.m.ButtonType.Default,
